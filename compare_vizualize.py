...
# ==========================================
# 3. THUẬT TOÁN DDPG & TD3 CÓ CHỨC NĂNG SAVE/LOAD
# ==========================================
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action):
...
# ==========================================
# 4. TÍNH TRUE Q-VALUE (GIỮ NGUYÊN)
# ==========================================
...
def train_agent(agent_name, AgentClass, env_name="Pendulum-v1",
    ...
    eval_steps, biases, estimated_qs, true_qs, reward_history = [], [], [], [], []
    episode_indices = []
    ...
    while total_timesteps < max_timesteps:
        ...
        if done:
            ...
            if episode_num % 20 == 0:
                print(f"  [{agent_name}] Episode: {episode_num} | Steps: {total_timesteps} | Reward: {episode_reward:.1f}")

            state, _ = env.reset()
            episode_reward = 0

        if total_timesteps % eval_freq == 0 and total_timesteps >= start_steps:
            est_q, true_q = compute_true_q_value(env_name, agent, num_episodes=10, discount=discount)
            bias = est_q - true_q
    ...
    # LƯU MODEL LẠI SAU KHI TRAIN XONG
    agent.save(f"./{agent_name}_model")
    print(f"\n✅ Đã lưu model của {agent_name} thành công!")
...
# ==========================================
# 6. HÀM HIỂN THỊ ĐỒ HỌA (VISUALIZE)
# ==========================================
def visualize_agent(agent_name, AgentClass, env_name="Pendulum-v1", episodes=3):
    print(f"\n👀 Đang tải Model và hiển thị Visualize cho {agent_name}...")
    
    # Cần render_mode='human' để xem trực quan
    ...
    # Load model
    try:
        agent.load(f"./{agent_name}_model")
        print(f"✅ Load {agent_name} model thành công!")
    except Exception as e:
        print(f"❌ Không tìm thấy model. Bạn cần Train trước. Chi tiết lỗi: {e}")
        env.close()
        return
    ...
    for ep in range(episodes):
        ...
        ep_reward = 0
        while not done:
            # Dùng policy thuần, không có noise khi test
            action = agent.select_action(np.array(state))
            state, reward, terminated, truncated, _ = env.step(action)
            ep_reward += reward
            done = terminated or truncated
            
        print(f"  [Visualize {agent_name}] Episode {ep+1}: Total Reward = {ep_reward:.2f}")

    env.close()


# ==========================================
# 7. MAIN SCRIPT & ĐIỀU KHIỂN LUỒNG CHẠY
# ==========================================
if __name__ == "__main__":
    ENV_NAME = "Pendulum-v1"
    MAX_TIMESTEPS = 100000  # Rút gọn xuống xíu nếu bạn muốn train lẹ, hoặc để nguyên 100k
    EVAL_FREQ = 2000
    DISCOUNT = 0.99

    # ==================================
    # CỜ ĐIỀU KHIỂN (THAY ĐỔI Ở ĐÂY)
    # ==================================
    # Đặt TRAIN_MODE = True lần đầu để học và lưu trọng số
    # Từ lần 2 trở đi đổi TRAIN_MODE = False để chỉ xem Visualize
    TRAIN_MODE = False
    VISUALIZE_MODE = True
    ...
    if TRAIN_MODE:
        ...
        # ========== VẼ BIỂU ĐỒ (Chỉ vẽ khi train) ==========
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))

        # ---- Biểu đồ 1: Overestimation Bias ----
        ax1 = axes[0, 0]
        ax1.plot(ddpg_steps, ddpg_bias, label='DDPG Bias', color='red', linewidth=2, marker='o', markersize=4, alpha=0.8)
        ax1.plot(td3_steps, td3_bias, label='TD3 Bias', color='blue', linewidth=2, marker='s', markersize=4, alpha=0.8)
        ...
        ax2 = axes[0, 1]
        ax2.plot(ddpg_steps, ddpg_est_q, label='DDPG Estimated Q', color='red', linewidth=2, linestyle='-')
        ax2.plot(ddpg_steps, ddpg_true_q, label='DDPG True Q (MC)', color='darkred', linewidth=2, linestyle='--')
        ax2.plot(td3_steps, td3_est_q, label='TD3 Estimated Q', color='blue', linewidth=2, linestyle='-')
        ax2.plot(td3_steps, td3_true_q, label='TD3 True Q (MC)', color='darkblue', linewidth=2, linestyle='--')
        ...
        ax3.plot(eps_ddpg, ddpg_reward_smooth, label='DDPG (Smoothed)', color='red', linewidth=2.5, linestyle='--')
        ax3.plot(eps_td3, td3_reward_smooth, label='TD3 (Smoothed)', color='blue', linewidth=2.5)
        ...
        plt.suptitle(f'So sánh Overestimation: DDPG vs TD3 trên {ENV_NAME}', fontsize=16, fontweight='bold', y=1.02)
        plt.tight_layout()
        plt.savefig('td3_vs_ddpg_overestimation.png', dpi=150, bbox_inches='tight')
        print("\n✅ Đã lưu biểu đồ: compare_visualize.png")
        plt.show()

    # ==========================================
    # CHẠY VISUALIZATION (Xem quá trình hoạt động)
    # ==========================================
    if VISUALIZE_MODE:
        # Chạy hiển thị cho DDPG 3 lần
        visualize_agent("DDPG", DDPG, env_name=ENV_NAME, episodes=3)
        # Chạy hiển thị cho TD3 3 lần
        visualize_agent("TD3", TD3, env_name=ENV_NAME, episodes=3)